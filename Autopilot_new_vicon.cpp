...
void Autopilot_loop(void *){ //runs inner loop and controls motor comamnds according to the control algorithm
	...
	while(1){
		...
		//attitude control
		attitude_control.pitch=vicon_control.pitch+innerloop_gain_pitch*rate_error.pitch;///pitch control
		attitude_control.roll=vicon_control.roll+innerloop_gain_roll*rate_error.roll;//roll control
		attitude_control.yaw=vicon_control.yaw+innerloop_gain_yaw*rate_error.yaw;//yaw control
